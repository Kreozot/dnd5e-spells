...
const EachLevelUpgrades: FC<Props & ReduxProps> = (props) => {
  const {
    initial, eachLevelInc, postfix = '', spellLevel, currentLevel, hint,
  } = props;
  ...
  const tooltipText = useMemo(() => {
    return (
      <>
        <span>
          { declension(initial, postfix) } at { spellLevel } spell slot level,
          on each next spell slot level increases by { declension(eachLevelInc, postfix) }
        </span>
        { Boolean(hint)
